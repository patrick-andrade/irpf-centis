...
- **Glossário por contrato.** `config/schema/indicators.csv` define cada indicador exibido — rótulo, definição, leitura, faixa, nota metodológica de cálculo com dados agrupados e chaves bibliográficas — e `config/schema/references.csv` projeta `reports/references.bib` num formato que o painel renderiza sem pandoc. `config/schema/rankings.csv` ganha `formula` e `reference_keys`. `test-glossary.R` falha se o seletor do painel, os contratos e a bibliografia divergirem.
- **Quadro "Conceitos e indicadores" na base de cada aba do painel**, montado a partir desses contratos. Em Evolução ele acompanha a seleção e traz a tabela dos onze conceitos de renda; nas demais abas explica os termos daquela leitura. Fica depois dos gráficos, para não empurrar a visualização para fora da primeira tela.
- **Aparato de rodapé padronizado** (`bloco_rodape()`): nota, observação e fonte, nesta ordem, colado ao conteúdo que qualifica. Substitui as notas soltas espalhadas pelos módulos.
- **Contagem de declarações por trás das participações de topo.** Nova tabela `top_group_counts` soma as folhas disjuntas acima de cada limiar e registra declarações, declarações conjuntas e dependentes. A aba "Topos da renda" passa a dizer, por exemplo, que em 2024 o top 0,01% no Brasil reúne 4.167 declarações, das quais 770 conjuntas, e 3.433 dependentes. Declarações conjuntas só existem na fonte a partir de 2023; nos anos anteriores o painel diz que o campo não é divulgado, em vez de exibir zero.
- **Aba "Bens e dívidas" separada em "Bens" e "Dívidas"**, sob o menu "Patrimônio e dívidas" da barra de navegação. Bens reúne a composição do patrimônio declarado, o patrimônio médio por posição na distribuição de renda e a série de ordenação direta nacional. Dívidas traz a dívida média por posição e a razão dívida/renda, com detalhe ordinal do topo em ambos.
- **`wealth_by_bin`**: bens e dívidas por grupo disjunto de RB4, com participação de cada grupo nos estoques e a razão dívida/renda. A razão fica `NA` onde a renda declarada do grupo é nula — 814 de 27.376 grupos —, pelo mesmo critério da alíquota efetiva. O total de bens é a soma das quatro famílias divulgadas, porque o consolidado só existe de 2022 em diante; onde os dois existem, o novo gate `asset_total_identity` confere um contra o outro. Gates novos também para razão negativa e para o aninhamento das contagens de topo.
- **Aba "Metodologia e dados" reformada**: universo, tabela dos conceitos de renda, ficha de cada indicador com a nota de cálculo agrupado e as referências, descrição do pipeline de dados curados com as decisões que ele não negocia, lista bibliográfica completa com DOI e links para o repositório, o site e as fontes oficiais. Downloads passam a incluir bens e dívidas por grupo, contagens de topo e o próprio glossário.

### Alterado

- O bundle do painel vai de 1,1 MB a 2,3 MB, pelas duas tabelas novas. Só as colunas que alguma aba desenha entram; a tabela completa fica em `data/processed/wealth-by-bin.parquet`. Primeira resposta local medida em 3,6 s, dentro do limite de 5 s.
- `docs/methodology.md` ganha a tabela dos conceitos de renda, as seções de endividamento e de contagens de topo, e a descrição do pipeline. `docs/limitations.md` ganha os itens 15 e 16.
